//
//  HMServciesCollectionView.swift
//  SampleSlideMenu
//
...
class HMServicesCollectionView: UICollectionView {
    ...
    func commonInit (navItem : NavigationItem) {
        ...
        let flowLayout: HMServicesCollectionViewFlowLayout  = HMServicesCollectionViewFlowLayout(grid: self, imageList: self.navigationItem.imageList!)
        flowLayout.extraSpace = 70
    ...
    func getCellFrameAtIndexPath(_ indexPath: IndexPath)-> CGRect {
        ...
        let x  = (indexPath.row) % 2 == 0 ? 10 : 180 // indexPath.row * Int(self.navigationItem.collectionViewSize.width) + (indexPath.row * self.navigationItem.spacing)

        if (indexPath.row)>1 {
            nextY = (indexPath.row) % 2 == 0 ? (nextY + 20 + Int(self.navigationItem.collectionViewSize.height)) : nextY
        }else  {
            nextY  = 0
        }
...
extension HMServicesCollectionView: UICollectionViewDataSource, UICollectionViewDelegate {
