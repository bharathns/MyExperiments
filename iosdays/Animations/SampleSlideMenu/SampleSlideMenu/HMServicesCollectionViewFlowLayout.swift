//
//  HMServicesCollectionViewFlowLayout.swift
//  SampleSlideMenu
//
...
class HMServicesCollectionViewFlowLayout: UICollectionViewFlowLayout {
    
    var grid : HMServicesCollectionView? = nil
    ...
    init(grid: HMServicesCollectionView, imageList: [String]) {
        super.init()
        setupLayout()
        if case self = self {
            self.grid = grid
    ...
    override func prepare() {
        ...
        if let grid:HMServicesCollectionView = self.grid {
            let noOfSections:Int = grid.numberOfSections
            for sectionIndex in 0..<noOfSections {
                let noOfItems:Int = grid.numberOfItems(inSection: sectionIndex)
                if noOfItems > 0 {
    ...
    override var collectionViewContentSize : CGSize {
        ...
        return CGSize(width: grid.frame.width, height: totalHeight + 180)
    }
}
